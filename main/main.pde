...
void draw() {
 ...
 // **05 implement corner operators in Mesh
  ...
  int line=0;
  scribeHeader(" Project 3 for Rossignac's 2018 Graphics Course CS6491 by Vinayak GARGYA ",line++);
  ...
  int v1 = 0; int v2 = 1; int prev = -1;
  float delta = 0;
  void moveMapleSyrup()
  {
   if(ang != PI/2 & ang != 3*PI/2)
     M.calculateHyperbolaParameters(ang);
   ang += delta;
