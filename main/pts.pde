...
class pts // class for manipulaitng and displaying pointclouds or polyloops in 3D 
  ...
  pts declare() 
    ...
    for (int i=0; i<maxnv; i++) R[i]=30 + random(40); 
  ...
  pts inflatePicked(float val) { println("Inflating by ",val);R[pv] += val; return this;}      // moves selected point (index p) by amount mouse moved recently
