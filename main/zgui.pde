void keyPressed() 
    ...
    if(key=='1') {step1=!step1; if(step1) {M.reset(); M.loadVertices(R.G,R.nv,R.R); M.triangulate();}}
    ...
    if(key=='c') {triangleN++; M.printTriangle(triangleN*3);} 
    ...
    if(key=='k') ; 
    if(key=='l') M.left();
    if(key=='m') {M.reset(); M.loadVertices(R.G,R.nv,R.R); M.triangulate(); M.computeO();}
    if(key=='n') M.next();
    if(key=='o') M.opposite();  
    if(key=='p') M.previous();
    if(key=='q') ; 
...
//  if (!keyPressed) {Of.add(ToIJ(V((float)(mouseX-pmouseX),(float)(mouseY-pmouseY),0))); }
  ...
  if (keyPressed && key=='s') {R.inflatePicked((mouseX-pmouseX)); }
  if (keyPressed && key=='v') {VOLUME += 1500*((mouseX-pmouseX)); println("Volume",VOLUME); }
...
String title ="Lattice Maker", name ="Vinayak Gargya",
